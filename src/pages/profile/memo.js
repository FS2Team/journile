...
class Memo extends React.Component<any, State> {
  ...
  renderPostsLoader() {
    ...
    return (
      <Query query={qGetUserPosts} variables={{user_id: user.id, date: posts.length > 0? posts[posts.length-1].original_post_date: null, isLater: false}}>
        {({ loading, error, data }) => {
          if (loading) return (<Spinner />)
          if (error) return <div></div>
    ...
    return (
      <div className='page'>
        <FloatingWrapper className='left-content detail'>
          ...
          {user.site_url &&
            <div className='line'>
              ...
              <a href={user.site_url} className='link' target="_blank" rel='noopener noreferrer'>{user.site_url}</a>
            </div>
          }
          <div className='line'>
          ...
          <div className='line'>
            ...
            <a href={`/ping/${user.username}`} className='label' target="_blank" rel='noopener noreferrer'>{'Ping'}</a>
          </div>
          <div className='line'>
            <Icon className='icon' name='location' size={24} />
            <span className='label'>{`${user.city? user.city.name : 'New York'}, ${user.country? user.country.name : 'United States'}`}</span>
          </div>
          <div className='line'>
            <Icon className='icon' name='calendar' size={24} />
            <span className='label'>{`Joined: ${formatDate(user.create_date, 'D MMMM YY')}`}</span>
          </div>
          <div className='line'>
        ...
        <div className='main-content post-list'>
          ...
          {isLoggedin && me && me.id === user.id && <Composer onIssue={post => this.setState({ posts: _.concat([post], posts) })} />}
...
export default connect(mapStateToProps, { toggleAuthModal })(Memo);